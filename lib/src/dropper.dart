...
class Dropper {
  ...
  void _paste(ClipboardEvent event) {
    ...
    if (blobs.isNotEmpty) {
      _listener.acceptBlobs(blobs
          .where((Blob f) => _listener.isMimeTypeAccepted(f.type))
  ...
  void _drop(MouseEvent e) {
    ...
    _listener.acceptBlobs(files
        .where((File f) => _listener.isMimeTypeAccepted(f.type))
        .toList(growable: false));
